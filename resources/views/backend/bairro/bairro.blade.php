                            ...
                            <tbody>
                                @foreach ($bairros as $bairro)
                                    <tr>
                                        ...
                                        <td>{{ ucfirst($bairro->uuid) }}</td>
                                        <td>{{ ucfirst($bairro->name) }}</td>
                                        @if ($bairro->province!= null)
                                            <td>{{ ucfirst($bairro->province->name) }}</td>
                                        @endif
                        ...
                        <div class="container form-group row">
                            ...
                            <div class="col-md-12">
                                <input id="name" type="text" class="form-control" name="name"
                                    placeholder="Digite nome de bairro" required autocomplete="name" autofocus>
                            </div>
                            <div class="col-md-12 py-3">
                                <label for="cidades">Cidade</label>
                                <select name="province_uuid" id="create_cidade" class="selectric">
                                    @foreach ($cidades as $cidade)
                                        <option value="{{$cidade->uuid}}">{{$cidade->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                ...
                <form action="{{ route('bairro.index') }}" method="post" id="edit_form">
                    ...
                    <div class="modal-body">
                        <div class="container form-group row">
                            ...
                            <label for="cidades">Cidade</label>
                            <select name="province_uuid" id="update_cidade" class="selectric">
                                @foreach ($cidades as $cidade)
                                    <option value="{{$cidade->uuid}}">{{$cidade->name}}</option>
                                @endforeach
                            </select>
    ...
    <script>
        ...
        $(function() {
            $('#EditBairro').on('show.bs.modal', function(event) {
                ...
                form.attr('action', '{!! route('bairro.index') !!}' + '/' + code.uuid);
                modal.find('#id_edit').val(code.uuid);
                modal.find('#name_edit').val(code.name);
                $("#update_cidade option").attr("selected", false);
                $('#update_cidade option[value="'+ code.province_uuid+'"]').attr("selected", "selected");

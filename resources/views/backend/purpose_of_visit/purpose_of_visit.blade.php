        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4>Tabela de objectivo das visitas</h4>
                    <div class="card-header-action">
                        <button class="btn btn-info" data-toggle="modal" data-target="#novoProvincia"><i
                                class="fas fa-plus"></i><span> Novo objectivo da visita</span></button>
                    </div>
                </div>
                <div class="card-body">
                            ...
                            <tbody>
                                @foreach ($purpose_of_visits as $purpose_of_visit)
                                    <tr>
                                        ...
                                        <td>{{ ucfirst($purpose_of_visit->uuid) }}</td>
                                        <td>{{ ucfirst($purpose_of_visit->name) }}</td>
                                        <td><button class="btn btn-secondary" data-toggle="modal" data-target="#EditProvincia"
                                                data-code="{{ ucfirst($purpose_of_visit) }}"><i class="fa fa-edit"></i></button>
                                        </td>
                                        <td><a href="#" class="btn btn-danger" data-toggle="modal"
                                                data-target="#DeleteProvincia" data-code="{{ ucfirst($purpose_of_visit) }}"><i
...
@section('modals')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Novo objectivo da visita</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('purpose_of_visit.store') }}" method="post">
                    @csrf
                        ...
                        <div class="container form-group row">
                            ...
                            <div class="col-md-12">
                                <input id="name" type="text" class="form-control" name="name"
                                    placeholder="Digite nome do objectivo da visita" required autocomplete="name" autofocus>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="EditProvincia">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Editar objectivo da visita</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('purpose_of_visit.index') }}" method="post" id="edit_form">
                    @csrf
                    @method('PATCH')
                    <div class="modal-body">
                        <div class="container form-group row">
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="DeleteProvincia">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title">Deletar objectivo da visita</h6>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('purpose_of_visit.index') }}" method="post" id="delete_form">
                    @csrf
                    @method('DELETE')
                    <div class="modal-footer bg-whitesmoke br">
...
@push('js')
    <script>
        $("#proviniencias").dataTable({
            ...
            "info": false,
            "language": {
                'sSearch': 'Pesquisar objectivo das visitas',
                "oPaginate": {
                    "sFirst": "Primero",
                    "sLast": "Último",
                    "sNext": "Siguinte",
        ...
        $(function() {
            $('#EditProvincia').on('show.bs.modal', function(event) {
                ...
                form.attr('action', '{!! route('purpose_of_visit.index') !!}' + '/' + code.uuid);

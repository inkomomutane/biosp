        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4>Tabela de proviniências</h4>
                    <div class="card-header-action">
                        <button class="btn btn-info" data-toggle="modal" data-target="#novoProvincia"><i
                                class="fas fa-plus"></i><span> Nova proviniência</span></button>
                    </div>
                </div>
                <div class="card-body">
                            ...
                            <tbody>
                                @foreach ($provenaces as $provenace)
                                    <tr>
                                        ...
                                        <td>{{ ucfirst($provenace->uuid) }}</td>
                                        <td>{{ ucfirst($provenace->name) }}</td>
                                        <td><button class="btn btn-secondary" data-toggle="modal" data-target="#EditProvincia"
                                                data-code="{{ ucfirst($provenace) }}"><i class="fa fa-edit"></i></button>
                                        </td>
                                        <td><a href="#" class="btn btn-danger" data-toggle="modal"
                                                data-target="#DeleteProvincia" data-code="{{ ucfirst($provenace) }}"><i
...
@section('modals')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Nova proviniência</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('provenace.store') }}" method="post">
                    @csrf
                        ...
                        <div class="container form-group row">
                            ...
                            <div class="col-md-12">
                                <input id="name" type="text" class="form-control" name="name"
                                    placeholder="Digite nome da proviniência" required autocomplete="name" autofocus>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="EditProvincia">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Editar proviniência</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('provenace.index') }}" method="post" id="edit_form">
                    @csrf
                    @method('PATCH')
                    <div class="modal-body">
                        <div class="container form-group row">
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="DeleteProvincia">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title">Deletar proviniência</h6>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('provenace.index') }}" method="post" id="delete_form">
                    @csrf
                    @method('DELETE')
                    <div class="modal-footer bg-whitesmoke br">
...
@push('js')
    <script>
        $("#proviniencias").dataTable({
            ...
            "info": false,
            "language": {
                'sSearch': 'Pesquisar proviniências',
                "oPaginate": {
                    "sFirst": "Primero",
                    "sLast": "Último",
                    "sNext": "Siguinte",
        ...
        $(function() {
            $('#EditProvincia').on('show.bs.modal', function(event) {
                ...
                form.attr('action', '{!! route('provenace.index') !!}' + '/' + code.uuid);

        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h4>Tabela de documentos necessários</h4>
                    <div class="card-header-action">
                        <button class="btn btn-info" data-toggle="modal" data-target="#novoProvincia"><i
                                class="fas fa-plus"></i><span> Nova documento necessário</span></button>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-striped v_center" id="document_types">
                            <thead>
                                <tr>
                            ...
                            <tbody>
                                @foreach ($document_types as $document_type)
                                    <tr>
                                        ...
                                        <td>{{ ucfirst($document_type->uuid) }}</td>
                                        <td>{{ ucfirst($document_type->name) }}</td>
                                        <td><button class="btn btn-secondary" data-toggle="modal" data-target="#EditProvincia"
                                                data-code="{{ ucfirst($document_type) }}"><i class="fa fa-edit"></i></button>
                                        </td>
                                        <td><a href="#" class="btn btn-danger" data-toggle="modal"
                                                data-target="#DeleteProvincia" data-code="{{ ucfirst($document_type) }}"><i
...
@section('modals')
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Novo documento necessário</h5>
                        ...
                        <div class="container form-group row">
                            ...
                            <div class="col-md-12">
                                <input id="name" type="text" class="form-control" name="name"
                                    placeholder="Digite nome do documento necessário" required autocomplete="name" autofocus>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="EditProvincia">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Editar documento necessário</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('document_type.index') }}" method="post" id="edit_form">
                    @csrf
                    @method('PATCH')
                    <div class="modal-body">
                        <div class="container form-group row">
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="DeleteProvincia">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h6 class="modal-title">Deletar documento necessário</h6>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('document_type.index') }}" method="post" id="delete_form">
                    @csrf
                    @method('DELETE')
                    <div class="modal-footer bg-whitesmoke br">
...
@push('js')
    <script>
        $("#document_types").dataTable({
            ...
            "info": false,
            "language": {
                'sSearch': 'Pesquisar documentos necessários',
                "oPaginate": {
                    "sFirst": "Primero",
                    "sLast": "Último",
                    "sNext": "Siguinte",
        ...
        $(function() {
            $('#EditProvincia').on('show.bs.modal', function(event) {
                ...
                form.attr('action', '{!! route('document_type.index') !!}' + '/' + code.uuid);
